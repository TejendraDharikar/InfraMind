    ...
    if (recentMetrics.length > 0) {
      const sum = recentMetrics.reduce(
        (acc, m) => ({
          ...
          network: acc.network + m.networkUsage,
      ...
      avgMetrics.memory =
        Math.round((sum.memory / recentMetrics.length) * 100) / 100;
      avgMetrics.network =
        Math.round((sum.network / recentMetrics.length) * 100) / 100;
      avgMetrics.disk =
        Math.round((sum.disk / recentMetrics.length) * 100) / 100;
    }
