...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({
      success: true, data: result.metrics,
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    res.status(201).json({ 
      success: true, 
      data: result, 
      message: result.alerts.length > 0 ? `Metric created. ${result.alerts.length} anomaly alert(s) generated.` : 'Metric created successfully',
      timestamp: new Date().toISOString() 
    });
  } catch (error: any) {
    if (error.name === 'ZodError') return res.status(400).json({ success: false, error: 'Validation Error', details: error.errors });
...
router.get('/server/:serverId/latest', async (req: Request, res: Response) => {
  try {
    ...
    const metrics = await metricsQuery.getLatestServerMetrics(req.params.serverId);
    res.status(200).json({ success: true, data: metrics, count: metrics.length, timestamp: new Date().toISOString() });
  } catch (error) {
