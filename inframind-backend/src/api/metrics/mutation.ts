  ...
  createMetric: async (data: CreateMetricBody) => {
    ...
    for (const anomaly of anomalies) {
      const alert = await prisma.alert.create({
        data: { serverId: data.serverId, type: anomaly.type, message: anomaly.message, severity: anomaly.severity, status: 'open' }
      });
    ...
    if (generatedAlerts.length > 0) {
      // Fire webhooks
      const activeWebhooks = await prisma.webhook.findMany({ where: { isActive: true } });
        ...
        if (hasCritical || webhook.events.includes('all')) {
          try {
            fetch(webhook.url, {
              ...
              body: JSON.stringify({ server: server.name, alerts: generatedAlerts })
    ...
    if (anomalies.some(a => a.severity === 'critical')) {
      const updatedServer = await prisma.server.update({ where: { id: data.serverId }, data: { status: 'unhealthy' } });
