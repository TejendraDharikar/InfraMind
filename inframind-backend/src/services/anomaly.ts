...
interface ThresholdRule {
  metric: keyof Pick<MetricInput, 'cpuUsage' | 'memoryUsage' | 'networkUsage' | 'diskUsage'>;
...
export function getRecommendation(type: string, severity: string): string {
  const recommendations: Record<string, Record<string, string>> = {
    cpu: {
      warning: 'Consider identifying CPU-intensive processes with `top` or `htop`. If sustained, evaluate horizontal scaling.',
      critical: 'Immediate action needed: Check for runaway processes, consider emergency scaling, or enable auto-scaling policies.',
    },
    memory: {
      warning: 'Review memory allocation. Check for memory leaks using profiling tools. Consider increasing swap or RAM.',
      critical: 'Risk of OOM kills. Restart memory-intensive services, increase instance memory, or enable memory limits.',
    },
    network: {
      warning: 'Monitor bandwidth patterns. Check for unexpected traffic spikes or potential DDoS activity.',
      critical: 'Network saturation detected. Enable rate limiting, check for DDoS, or upgrade network capacity.',
    },
    disk: {
      warning: 'Clean up old logs, temporary files, and unused data. Consider expanding storage volume.',
      critical: 'Disk nearly full. Immediately free space by clearing logs/temp files. Expand volume ASAP to avoid service disruption.',
    },
  };

  return recommendations[type]?.[severity] || 'Monitor the situation and take action if metrics continue to deteriorate.';
}
